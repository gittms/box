...
namespace Definitif.Data.Test
{
    ...
    public class DrawTest
    {
        ...
        {
            query = new Update<Models.Chair>().Set(m => m.C.Name == "Small Chair").Where(m => m.C.Id < 10);
            ...
            query = new Update<Models.Chair>().Set(m => m.C.Name == "Big table").Where(m => m.C.Table.Id == select);
            ...
            // Paged updates.
            query = new Update<Models.Table>().Set(m => m.C.Name == "Big table").Where(m => m.C.Id > 10).Top(10).OrderBy(m => m.C.Id.Asc);
